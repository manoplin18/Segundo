...
int main(int argc, char *argv[]){
    ...
    cout << "Fechas:"; f1.ver();cout << ", ";f2.ver();cout << ", ";f3.ver();cout << ", "; f4.ver();cout << endl;
    if (f3.bisiesto()&& !f2.bisiesto()&& f4.bisiesto()==false)
        cout << f3.getAnio() << "esbisiesto, " << f2.getAnio()<< " y " << f4.getAnio()<< " no\n";
    ...
    */
    ...
    ContratoTP ct3(ct1);
    ContratoMovil cm1(17333256, f1, 0.12, 100, "ESPANIOL"); //habla 100 minutos
    ...
    cout << setprecision(2) << fixed;//a partir de aqui float se muestracon 2 decimales
    ...
    Yoigo.ver();            //tener que meter datoscada vez que pruebo el programa
    cout <<"\nYoigo tiene " << Yoigo.nContratosTP() << " Contratos de Tarifa Plana\n\n";

    Yoigo.crearContrato();
    Yoigo.crearContrato();

    ok = Yoigo.cancelarContrato(28);
    if (ok) cout << "\nContrato 28 cancelado\n";
    else cout << "\nEl Contrato 28 no existe\n";

    ok = Yoigo.cancelarContrato(4);  //esteContratosi existe
    if (ok) cout << "Contrato 4 cancelado\n";
    else cout << "El Contrato 4 no existe\n";

    Yoigo.bajaCliente(75547001);  //debe eliminar el cliente y sus 3 Contratos
